...
public class DayAndNightControl : MonoBehaviour {
	...
	void Start () {
		...
		targetCam = GM.Joueur.playerCam.GetComponent<Camera>();
		time = MyTimeManager.timeInstance;
		time.GHourPassed += OnGHourPassed;
		/*foreach (Camera c in GameObject.FindObjectsOfType<Camera>()) //j'ai change pcq c pas efficient
		{
    ...
    {
		currentTime = (float)time.Hour*0.04f;
		UpdateLight();
	}
